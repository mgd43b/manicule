...
class VerificationRun:
    ...
    async def aclose(self, deadline_s: float = CLOSE_DEADLINE_S) -> None:
        ...
        **Settling here is not belt-and-braces.** Canceling a task that has not yet had its
        ...
        able to stall request teardown indefinitely. The one step it does not cut short is a
        connection the engine is still opening for a blob read: the engine finishes opening it,
        and closes it, before raising the cancellation, because a connection canceled half-open
        is stranded where nothing can close it (``docs/storage.md`` §3.1).
        """
