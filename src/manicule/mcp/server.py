...
**And why a socket may carry it at all.** The gate is one layer up:
:func:`~manicule.app.bind.resolve_bind` refuses a non-loopback bind unless an operator named a
host and passed an explicit opt-in. Nobody acquires a network surface by accident, so a scoped
write on it sits behind a door that is already locked — and the deployment this exists for is
