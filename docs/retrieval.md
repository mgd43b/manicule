...
| **Query decomposition** | Only meaningful on multi-hop questions. The labeled subset this asked for now exists — [`evaluation/multi-hop-queries.json`](evaluation/multi-hop-queries.json), authored over this repository's own `docs/`, every query naming the passages a correct answer joins. What remains is the measurement: nDCG@10 **on that set alone**, since averaging it into the full set will hide the effect either way. It has not been run |
